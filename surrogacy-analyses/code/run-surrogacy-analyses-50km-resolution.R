#' ---
#' title: Global Reptile Assessment <br> <strong> Surrogacy in Conservation For Threatened Species - 50km resolution analyses
#' ---
#' 
#' ## Set things up
...
dir.create("data/input_rasters/50/combined")
#'
#' ## Load cell-by-species matrices
threatened_reptiles_pa_matrix50 <- readRDS("data/threatened_amphibians_pa_matrix50.rds")
threatened_birds_pa_matrix50 <- readRDS("data/threatened_birds_pa_matrix50.rds")
threatened_mammals_pa_matrix50 <- readRDS("data/threatened_mammals_pa_matrix50.rds")
threatened_amphibians_pa_matrix50 <- readRDS("data/threatened_amphibians_pa_matrix50.rds")
#'
#' ## Generate rasters from cell-by-species matrices
#' ### Reptiles
purrr::map(names(threatened_reptiles_pa_matrix50)[3:4], function(sp){
...
ranks_random_rept <- random_ranks(ranks_CAZ_rept, runs = 50)
ranks_random_bird <- random_ranks(ranks_CAZ_bird, runs = 50)
ranks_random_mamm <- random_ranks(ranks_CAZ_mamm, runs = 50)
ranks_random_amph <- random_ranks(ranks_CAZ_amph, runs = 50)
...
map_extent <- purrr::map(vert_classes, extract_zonation_maps, algorithm = "CAZ", out_dir = "output/zonation_runs_threatened", suffix = "082020", resolution = "50") %>% 
...
ranks_random_terrestrial <- random_ranks(terrestrial_extent$cellID, runs = 50)
#' #### Threatened Reptile+Birds extent
ranks_random_rept_bird <- random_ranks(as.data.frame(map_extent[["Birds"]] > 0 | map_extent[["Reptiles"]] > 0, xy = TRUE) %>% mutate(cellID = 1:length(x[])) %>% dplyr::filter(complete.cases(.)) %>% pull(cellID), runs = 50)
#' #### Threatened Reptile+Mammals extent
ranks_random_rept_mamm <- random_ranks(as.data.frame(map_extent[["Mammals"]] > 0 | map_extent[["Reptiles"]] > 0, xy = TRUE) %>% mutate(cellID = 1:length(x[])) %>% dplyr::filter(complete.cases(.)) %>% pull(cellID), runs = 50)
#' #### Threatened Reptile+Amphibians extent
ranks_random_rept_amph <- random_ranks(as.data.frame(map_extent[["Amphibians"]] > 0 | map_extent[["Reptiles"]] > 0, xy = TRUE) %>% mutate(cellID = 1:length(x[])) %>% dplyr::filter(complete.cases(.)) %>% pull(cellID), runs = 50)
#' #### Threatened Birds+Mammals extent
ranks_random_bird_mamm <- random_ranks(as.data.frame(map_extent[["Birds"]] > 0 | map_extent[["Mammals"]] > 0, xy = TRUE) %>% mutate(cellID = 1:length(x[])) %>% dplyr::filter(complete.cases(.)) %>% pull(cellID), runs = 50)
#' #### Threatened Birds+Amphibians extent
ranks_random_bird_amph <- random_ranks(as.data.frame(map_extent[["Birds"]] > 0 | map_extent[["Amphibians"]] > 0, xy = TRUE) %>% mutate(cellID = 1:length(x[])) %>% dplyr::filter(complete.cases(.)) %>% pull(cellID), runs = 50)
#' #### Threatened Mammals+Amphibians extent
ranks_random_mamm_amph <- random_ranks(as.data.frame(map_extent[["Mammals"]] > 0 | map_extent[["Amphibians"]] > 0, xy = TRUE) %>% mutate(cellID = 1:length(x[])) %>% dplyr::filter(complete.cases(.)) %>% pull(cellID), runs = 50)
#'
#' ## Generate Surrogacy Curves
#' ### CAZ
#' #### Reptiles as target
#' ##### Optimal curve
curve_CAZ_rept_threat_rept <- get_sai_curves_target(target_matrix = threatened_reptiles_pa_matrix50, target_ranks = ranks_CAZ_rept_threat, algorithm = "CAZ", runs = 5)
saveRDS(curve_CAZ_rept_threat_rept, "output/zonation_runs_threatened/50/curve_CAZ_rept_threat_rept.rds")
#' ##### Birds as surrogate
curve_CAZ_rept_threat_bird <- get_sai_curves_surrogate(target_matrix = threatened_reptiles_pa_matrix50, surrogate_ranks = ranks_CAZ_bird_threat, algorithm = "CAZ", runs = 5)
saveRDS(curve_CAZ_rept_threat_bird, "output/zonation_runs_threatened/50/curve_CAZ_rept_threat_bird.rds")
#' ##### Mammals as surrogate
curve_CAZ_rept_threat_mamm <- get_sai_curves_surrogate(target_matrix = threatened_reptiles_pa_matrix50, surrogate_ranks = ranks_CAZ_mamm_threat, algorithm = "CAZ", runs = 5)
saveRDS(curve_CAZ_rept_threat_mamm, "output/zonation_runs_threatened50/curve_CAZ_rept_threat_mamm.rds")
#' ##### Amphibians as surrogate
curve_CAZ_rept_threat_amph <- get_sai_curves_surrogate(target_matrix = threatened_reptiles_pa_matrix50, surrogate_ranks = ranks_CAZ_amph_threat, algorithm = "CAZ", runs = 5)
saveRDS(curve_CAZ_rept_threat_amph, "output/zonation_runs_threatened50/curve_CAZ_rept_threat_amph.rds")
#' ##### Combination as surrogate
curve_CAZ_rept_threat_comb <- get_sai_curves_surrogate(target_matrix = threatened_reptiles_pa_matrix50, surrogate_ranks = ranks_CAZ_comb, algorithm = "CAZ", runs = 5)
saveRDS(curve_CAZ_rept_threat_amph, "output/zonation_runs_threatened50/curve_CAZ_rept_threat_amph.rds")
#'
#' ### ABF
#' #### Reptiles as target
#' ##### Optimal curve
curve_ABF_rept_threat_rept <- get_sai_curves_target(target_matrix = threatened_reptiles_pa_matrix50, target_ranks = ranks_ABF_rept_threat, algorithm = "ABF", runs = 5)
saveRDS(curve_ABF_rept_threat_rept, "output/zonation_runs_threatened50/curve_ABF_rept_threat_rept.rds")
#' ##### Birds as surrogate
curve_ABF_rept_threat_bird <- get_sai_curves_surrogate(target_matrix = threatened_reptiles_pa_matrix50, surrogate_ranks = ranks_ABF_bird_threat, algorithm = "ABF", runs = 5)
saveRDS(curve_ABF_rept_threat_bird, "output/zonation_runs_threatened50/curve_ABF_rept_threat_bird.rds")
#' ##### Mammals as surrogate
curve_ABF_rept_threat_mamm <- get_sai_curves_surrogate(target_matrix = threatened_reptiles_pa_matrix50, surrogate_ranks = ranks_ABF_mamm_threat, algorithm = "ABF", runs = 5)
saveRDS(curve_ABF_rept_threat_mamm, "output/zonation_runs_threatened50/curve_ABF_rept_threat_mamm.rds")
#' ##### Amphibians as surrogate
curve_ABF_rept_threat_amph <- get_sai_curves_surrogate(target_matrix = threatened_reptiles_pa_matrix50, surrogate_ranks = ranks_ABF_amph_threat, algorithm = "ABF", runs = 5)
saveRDS(curve_ABF_rept_threat_amph, "output/zonation_runs_threatened50/curve_ABF_rept_threat_amph.rds")
#' ##### Combination as surrogate
curve_ABF_rept_threat_comb <- get_sai_curves_surrogate(target_matrix = threatened_reptiles_pa_matrix50, surrogate_ranks = ranks_ABF_comb, algorithm = "ABF", runs = 5)
saveRDS(curve_ABF_rept_threat_amph, "output/zonation_runs_threatened50/curve_ABF_rept_threat_amph.rds")
#'
#'
...
#' #### CAZ
#' ##### Terrestrial extent
curve_CAZ_rept_threat_random_terr_ext <- get_sai_curves_random(target_matrix = threatened_reptiles_pa_matrix50, random_ranks = ranks_random_terrestrial, reference_coordinates = ranks_CAZ_rept_threat$reference_coordinates, algorithm = "CAZ", runs = 10)
saveRDS(curve_CAZ_rept_threat_random_terr_ext, "output/zonation_runs_threatened50/curve_CAZ_rept_threat_random_terr_ext.rds")
#' ##### Birds as surrogate
curve_CAZ_rept_threat_random_bird <- get_sai_curves_random(target_matrix = threatened_reptiles_pa_matrix50, random_ranks = ranks_random_rept_bird, reference_coordinates = ranks_CAZ_rept_threat$reference_coordinates, algorithm = "CAZ", runs = 10)
saveRDS(curve_CAZ_rept_threat_random_bird, "output/zonation_runs_threatened50/curve_CAZ_rept_threat_random_bird.rds")
#' ##### Mammals as surrogate
curve_CAZ_rept_threat_random_mamm <- get_sai_curves_random(target_matrix = threatened_reptiles_pa_matrix50, random_ranks = ranks_random_rept_mamm, reference_coordinates = ranks_CAZ_rept_threat$reference_coordinates, algorithm = "CAZ", runs = 10)
saveRDS(curve_CAZ_rept_threat_random_mamm, "output/zonation_runs_threatened50/curve_CAZ_rept_threat_random_mamm.rds")
#' ##### Amphibians as surrogate
curve_CAZ_rept_threat_random_amph <- get_sai_curves_random(target_matrix = threatened_reptiles_pa_matrix50, random_ranks = ranks_random_rept_amph, reference_coordinates = ranks_CAZ_rept_threat$reference_coordinates, algorithm = "CAZ", runs = 10)
saveRDS(curve_CAZ_rept_threat_random_amph, "output/zonation_runs_threatened50/curve_CAZ_rept_threat_random_amph.rds")
#' ##### Combined as surrogate
curve_CAZ_rept_threat_random_comb <- get_sai_curves_random(target_matrix = threatened_reptiles_pa_matrix50, random_ranks = ranks_random_rept_comb, reference_coordinates = ranks_CAZ_rept_threat$reference_coordinates, algorithm = "CAZ", runs = 10)
saveRDS(curve_CAZ_rept_threat_random_comb, "output/zonation_runs_threatened50/curve_CAZ_rept_threat_random_comb.rds")
#' #### ABF
#' ##### Terrestrial extent
curve_ABF_rept_threat_random_terr_ext <- get_sai_curves_random(target_matrix = threatened_reptiles_pa_matrix50, random_ranks = ranks_random_terrestrial, reference_coordinates = ranks_ABF_rept_threat$reference_coordinates, algorithm = "ABF", runs = 10)
saveRDS(curve_ABF_rept_threat_random_terr_ext, "output/zonation_runs_threatened50/curve_ABF_rept_threat_random_terr_ext.rds")
#' ##### Birds as surrogate
curve_ABF_rept_threat_random_bird <- get_sai_curves_random(target_matrix = threatened_reptiles_pa_matrix50, random_ranks = ranks_random_rept_bird, reference_coordinates = ranks_ABF_rept_threat$reference_coordinates, algorithm = "ABF", runs = 10)
saveRDS(curve_ABF_rept_threat_random_bird, "output/zonation_runs_threatened50/curve_ABF_rept_threat_random_bird.rds")
#' ##### Mammals as surrogate
curve_ABF_rept_threat_random_mamm <- get_sai_curves_random(target_matrix = threatened_reptiles_pa_matrix50, random_ranks = ranks_random_rept_mamm, reference_coordinates = ranks_ABF_rept_threat$reference_coordinates, algorithm = "ABF", runs = 10)
saveRDS(curve_ABF_rept_threat_random_mamm, "output/zonation_runs_threatened50/curve_ABF_rept_threat_random_mamm.rds")
#' ##### Amphibians as surrogate
curve_ABF_rept_threat_random_amph <- get_sai_curves_random(target_matrix = threatened_reptiles_pa_matrix50, random_ranks = ranks_random_rept_amph, reference_coordinates = ranks_ABF_rept_threat$reference_coordinates, algorithm = "ABF", runs = 10)
saveRDS(curve_ABF_rept_threat_random_amph, "output/zonation_runs_threatened50/curve_ABF_rept_threat_random_amph.rds")
#' ##### Combination as surrogate
curve_ABF_rept_threat_random_comb <- get_sai_curves_random(target_matrix = threatened_reptiles_pa_matrix50, random_ranks = ranks_random_rept_comb, reference_coordinates = ranks_ABF_rept_threat$reference_coordinates, algorithm = "ABF", runs = 10)
saveRDS(curve_ABF_rept_threat_random_comb, "output/zonation_runs_threatened50/curve_ABF_rept_threat_random_comb.rds")
#'
